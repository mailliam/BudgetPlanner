# BudgetPlanner
ITK aine I200 raames kirjutatud projekt
Maila Keerus

##Eesmärk
Luua vahend kulutuste kirjapanekuks ning nende jälgimiseks

##Kasutusjuhend kasutajale
1. Esmalt registreeri ennast programmi kasutajaks, seejärel logi sisse

2. Järgnevalt avanevas aknas hakkavad (pärast esimeste kulude sisestamist) näha olema viimaste kuude sisestatud kulutused kategooriate kaupa
	* perioodi saab valida View menüüst --> valikus on 3, 6 või 12 kuud

3. Kulusid saab sisestada File menüüst --> New Purchase.
   	* täita tuleb ostu andmed - ostja, kauplus, kuupäev
	* täita tuleb ostukorvi andmed - ese, kategooria, kogus, hind

  3.1. Ostukorvi täitmine
	* eseme esmakordsel sisestamisel tuleb valida kategooria, mis on edaspidi selle eseme grupeerimise aluseks.
	  Järgnevatel sama eseme ostudel tekib kategooria automaatselt ning ostu sisestamise käigus seda muuta ei saa.
	* rea summa arvutatakse automaatselt sisestatud koguse ning hinna põhjal
	* uue rea saamiseks vajuta pärast hinna sisestamist "Enter"
	* sisestamise lõppedes kontrolli, kas sisestatud ostukorvi kogusumma on õige (Total purchase amount)
	* salvesta ost

 
	